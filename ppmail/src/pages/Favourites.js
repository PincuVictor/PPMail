...
function Favourites() {
  ...
  const q = query(collection(db, 'mails'), where('recipient', '==', currentUser.email), where('favourite', '==', true), orderBy('createdAt', 'desc'))
  ...
  if (loaded) {
    ...
    <>
    <AuthProvider>
        <strong style={{display:'flex', justifyContent:'center', alignItems:'center', color:'#FFF', fontSize:'32px', zIndex:5, marginTop:13, marginBottom:-63}}>Favourites</strong>
        <Sidebar />
        <ul className='mails'>
